...
import com.sportsexp.common.util.pojo.Ftp;
public class FtpUtils {
				...
				if (changedir) {
					...
					for (int i = 0; i < files.length; i++) {
						try {
							...
							logger.debug(e);
							logger.debug("<" + files[i].getName() + ">下载失败");
						}
					}
				}
			} catch (Exception e) {
				logger.debug(e);
				logger.debug("下载过程中出现异常");
			}
		} else {
			logger.debug("链接失败！");
		}
	...
	public static void main(String[] args) throws Exception {
		Ftp f = new Ftp();
		f.setIpAddr("192.168.1.30");
		f.setUserName("tech");
		f.setPwd("1234");
		f.setPath("/FAXDATA/sfax/workfacorty/20160623");
		FtpUtils.connectFtp(f);
		File file = new File("D:/test/pd4ml.pdf");
		FtpUtils.upload(file);// 把文件上传在ftp上
		FtpUtils.closeFtp();
		// FtpUtils.startDown(f, "d:/temp", "/pd4ml.pdf");//下载ftp文件测试
